...
function initialize() {
	...
	try {
		...
		// create the armizare named object
		useLibrary( 'res://namedobj.js' );
		const armizare = Eons.namedObjects.armizare;

		// register armizare as a game supported by Strange Eons
		Game.register('armizare', @armizare_game, #armizare_game, ImageUtils.get( 'plugin.png' ), template);
				
		// now that the game is regsietered, we can add the
		// default settings to use for new cards
		Game.get( 'armizare' ).masterSettings.addSettingsFrom( 'base.settings' );
				
		ClassMap.add( 'base.classmap' );

	} catch( ex ) {
		Eons.log.severe( 'Armizare plug-in failed to start' );
